...
- `run-check`와 `run-preflight`는 완료된 Build/QA Run의 `delegation_records`가 worker 완료 상태만 기록하고 `orchestrator_verification` 또는 최종 `verified/needs_review/blocked` 정리가 없으면 경고한다.
