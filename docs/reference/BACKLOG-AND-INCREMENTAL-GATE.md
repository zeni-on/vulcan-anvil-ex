...
해당 프로젝트는 Gate 5 이후 `docs/06-backlog/BACKLOG.md`를 즉석에서 도입했고, 실전 운영에서 Backlog가 단순한 TODO 리스트가 아니라 **"작업 크기에 맞는 최소 프로세스를 선택하는 라우터"** 역할을 한다는 것이 드러났다.
...
본 문서의 결정은 Vulcan-Anvil **v1.1부터** 적용된다. v1.0에서 `docs/06-backlog/`를 수동으로 운영하던 기존 프로젝트는 v1.1 템플릿으로 마이그레이션할 때 기존 BACKLOG.md를 새 규약에 맞게 재분류해야 한다 (특히 Trivial 기준 변경).
