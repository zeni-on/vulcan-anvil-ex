...
| `Promote to product/audit` | 제품화 또는 감리형 프로젝트로 승격한다. |

## 3. 승격 레이어

PoC 결과가 유효하다고 해서 바로 audit profile로 가야 하는 것은 아니다.
대부분의 제품형 작업에서는 중간에 product profile을 두는 편이 자연스럽다.
...
Product profile은 audit의 가벼운 버전이 아니라 제품화/운영 가능 기준선이다.
PoC에서 확인한 가설과 데모 코드를 바로 감리 문서로 늘리기 전에, product 단계에서 다음을 정리한다.
...
권장 흐름은 `promote-profile --to product|audit --dry-run` 같은 후보 명령으로 부족한 산출물, ID, 테스트, 증적을 보고하고, Orchestrator가 사용자 승인 후 보강 Run을 만든다.
...
- PoC 결과를 product/audit로 승격할 때는 `@playwright/test`, `npx playwright test`, report/trace/screenshot 기준의 공식 UI 증적으로 보강한다.
- audit/product profile의 Gate 4 UI Pass는 커스텀 Playwright library script만으로 확정하지 않는다.

## 6. CLI와 검사 영향

PoC 전용 템플릿 세트를 도입하면 CLI도 profile별 필수 산출물 원장을 다르게 봐야 한다.

새 PoC 프로젝트는 `docs/poc/` 3종을 공식 작업 문서로 생성한다. Audit 산출물 템플릿 파일은 기본 생성하지 않고, Product/Audit 승격 판단 시 후보 문서와 Gap Report로 만든다.
...
Audit/Product에서는 공식 계약 skeleton이 필요하거나 환경 준비가 기능 구현 수준으로 커질 때만 `BW-000`을 사용한다.
...
- PoC 성공 후 product/audit 승격 경로를 둔다.

수정해야 할 부분:

- 원 제안의 중첩 Markdown code fence 예시는 실제 템플릿 파일로 옮길 때 깨질 수 있으므로 4중 fence 또는 개별 파일로 분리한다.
- `node scripts/run-e2e.js` 같은 커스텀 Playwright script는 PoC smoke/demo로는 허용하되 audit/product UI Pass로 쓰지 않는다.
...
5. PoC 결과를 `Continue`, `Pivot`, `Stop`, `Promote to product/audit` 중 하나로 정리할 수 있는지 확인한다.
...
- PoC -> Product 승격 Gap report
- Product -> Audit 승격 Gap report
