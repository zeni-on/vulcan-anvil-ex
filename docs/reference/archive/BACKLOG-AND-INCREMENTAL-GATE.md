...
> 상태: 보존 문서(Archive)
>
> 이 문서는 Vulcan-Anvil v1.1 시점의 Backlog Phase와 증분 Gate Rollback 도입 배경을 기록한 과거 의사결정 문서다.
> Vulcan-Anvil Ex의 현재 운영 기준은 `docs/backlog/PROCESS.md`, `docs/core/CHANGE_CONTROL_PROCESS.md`, `templates/GATE_GUIDE.md`를 우선한다.

> 본 문서는 Vulcan-Anvil v1.1 고도화(Backlog Phase 도입, 증분 Gate Rollback, Trivial 재정의)의 배경과 의사결정을 기록한다.
> 작성일: 2026-04-11 · 출처: 실제 프로젝트에서의 Backlog 운영 경험
...
**실제 프로젝트에서 관찰된 현상** (약 60개 요구사항 규모의 웹 애플리케이션):
...
해당 프로젝트는 Gate 5 이후 `docs/backlog/BACKLOG.md`를 즉석에서 도입했고, 실전 운영에서 Backlog가 단순한 TODO 리스트가 아니라 **"작업 크기에 맞는 최소 프로세스를 선택하는 라우터"** 역할을 한다는 것이 드러났다.
...
9개의 실제 Backlog 항목을 재검증하는 과정에서 세 가지 구조적 문제가 드러났다.
...
Backlog가 **"Gate 4에서 타협하는 핑계"**가 될 위험이 있다. 실제 사례: 페이지네이션을 "unwrap으로 동작만" 상태에서 Gate 4를 통과시키고 BL-003으로 넘긴 케이스가 대표적이다. 이것이 반복되면 Gate 4는 "진짜 검증"이 아니라 "다음 배치로 미루는 의식"이 된다.
...
| # | 항목 | 변경 |
|---|------|------|
| 1 | Backlog Phase 승격 | `templates/docs/backlog/` 추가, vulcan.py에 `backlog` CRUD 명령 도입 |
...
본 문서의 결정은 Vulcan-Anvil **v1.1부터** 적용된다. v1.0에서 `docs/backlog/`를 수동으로 운영하던 기존 프로젝트는 v1.1 템플릿으로 마이그레이션할 때 기존 BACKLOG.md를 새 규약에 맞게 재분류해야 한다 (특히 Trivial 기준 변경).
