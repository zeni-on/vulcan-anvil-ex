...
#### Product 3구간 반복 프로세스 (2026-09-11, 설계 검토 완료/runtime 미구현)
...
- 다음 순서: 상태 모델·범위/승인 계약 테스트 → Product 검사·Core/Codex/Gemini/선택 양식·Dashboard 연결 → 합성 프로젝트 반복/명시 이행 보존 검증 → 실제 적용 판단. 새 `init` 기본값이나 기존 프로젝트 상태를 먼저 바꾸지 않는다.
