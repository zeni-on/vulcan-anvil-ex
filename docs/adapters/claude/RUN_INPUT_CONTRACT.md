...
| `ui_implementation_contract_policy` | 화면 작업 시 | UIREF/prototype을 구현 계약으로 전환·판정 기준 |
...
# UIREF/prototype이 있는 화면 작업 시 사용
ui_implementation_contract_policy:
  ...
  impl_checklist:
    ...
    - "prototype CSS 또는 동등한 레이아웃/class 구조 재사용 여부 기록"
...
question_policy:
  ask_when:
    ...
    - "UIREF/prototype은 있는데 UI Implementation Contract가 비어 있어 비교 기준을 정할 수 없다."
...
- **`ui_implementation_contract_policy`**: prototype 또는 외부 시안이 있는 화면 작업에서 사용. Gate 2에서 시안을 참고자료 vs 구현 계약으로 분류하고, 구현 계약이면 필수 유지/변경 허용/금지/비교 방식 작성. Gate 3에서 UI 테스트 기대결과 반영. Impl 체크리스트로 확인. Gate 4에서 기준 UIREF와 구현 screenshot 차이를 `Pass`, `FIND`, `CR`로 판정.
