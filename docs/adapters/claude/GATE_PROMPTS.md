...
- **UI Implementation Contract (G2-03 화면 단계 필수, 화면 퍼블리싱 산출물/UIREF가 있을 때)**: 화면 퍼블리싱 산출물/이미지 시안/Figma/기존 화면 캡처가 단순 참고자료인지 구현 기준인지 분류한다. 구현 기준이면 UI Implementation Contract를 작성한다 — 기준 파일/CSS, 필수 유지 요소, 변경 허용 항목, 변경 금지 항목, 비교 방식. (`RUN_INPUT_CONTRACT.md` `ui_implementation_contract_policy` 참조)
- **상태별 UI 증적 기준**: UI 증적은 화면 단위가 아니라 기본/로딩/오류/성공/전환 상태별로 `UI-001-01` 형식으로 분리한다. 각 UI-ID에 기대 화면(UIREF/ui-baseline 경로)과 캡처 경로를 1:1로 둔다.
...
- UIREF/ui-baseline이 있는 SCR은 UI Implementation Contract가 작성되어 Gate 3/Impl/Gate 4에서 검증 가능한 비교 기준이 있음
...
- **UI 테스트**: UI-ID는 SCR 단위가 아니라 SCR×상태 단위(`UI-001-01`)로 작성. 각 UI-ID에 기대 화면(UIREF/ui-baseline) + 캡처 경로 1:1. 화면 퍼블리싱 기반이면 UI Implementation Contract의 필수 유지/변경 허용/금지 항목을 기대결과에 반영.
...
4. 개발표준정의서는 구현 계약으로 취급한다. 패키지 구조, 계층 책임, 로깅, 주석, 예외/메시지, 테스트 명령 기준이 비어 있으면 구현하지 않고 FIND/CR로 보고한다.
5. Spring Boot 구현은 개발표준의 base package와 feature 우선 구조를 따른다. `domain/{feature}` 래퍼는 DDD 구조 선택 사유가 개발표준에 있을 때만 사용한다.
6. scope.writable 안에서 현재 Build Wave만 구현한다.
7. 구현 파일과 테스트 파일에 관련 추적 ID를 남긴다.
8. 테스트와 린트를 실행한다 (Bash tool 직접 사용).
9. 화면이 있으면 Playwright로 UI 캡처 증적을 생성한다.
10. Playwright가 설치되어 있지 않으면 `npm install -D @playwright/test`와 `npx playwright install`을 실행한 뒤 `npx playwright test`로 재검증한다.
11. 테스트 결과서와 요구사항추적표에 증적을 연결한다.
...
- **Playwright 필수**: 화면 캡처 증적은 Playwright 실행 결과를 기준으로 한다. CDP, Claude-in-Chrome, 브라우저 수동 캡처만으로 UI Pass를 확정하지 않는다.
- **UI Implementation Contract 확인**: 화면 구현 전 관련 SCR의 UI Implementation Contract와 Gate 3 UI 테스트 기준을 확인. 화면 퍼블리싱 기반 화면은 기준 UIREF screenshot과 구현 screenshot의 차이를 기록하고 허용 여부 판정.
...
3. Playwright로 화면 스크린샷을 캡처한다. Playwright가 설치되어 있지 않으면 설치 후 재실행한다.
...
- **UI Contract 차이 판정**: 화면 퍼블리싱 산출물/UIREF가 있는 SCR은 기준 UIREF와 구현 screenshot의 차이를 `Pass` / `FIND` / `CR` 중 하나로 판정. 차이 없음을 묵시적으로 Pass 처리하지 않는다.
