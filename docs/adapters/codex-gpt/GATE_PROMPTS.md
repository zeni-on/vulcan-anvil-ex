...
4. 화면이 있으면 UI-ID, viewport, 기준 시안, 실제 캡처 경로, 비교 기준을 정의한다.
...
4. 테스트와 린트를 실행한다.
5. 화면이 있으면 기준 시안 또는 화면설계서를 기준으로 desktop/mobile 구현 화면을 확인한다.
6. Playwright 등 브라우저 검증 도구로 UI 캡처 증적을 생성한다.
7. 테스트 결과서와 요구사항추적표의 증적에 구현 파일, 테스트 파일, 화면 증적을 연결한다.
