...
9. 요구사항추적표, 테스트 결과서, Run 기록에 구현 파일, 명령 실행 결과, 테스트 결과를 연결한다.
10. 작업자 runner로 실행 중이면 `vulcan.py wave-complete <BW-ID>`를 직접 실행하지 않고 Orchestrator에게 실행 필요 여부를 보고한다. Orchestrator가 직접 수행하는 Run이면 `wave-complete`로 session 상태를 갱신한다.
11. 커밋 후보 메시지를 보고한다. 사용자가 승인하거나 자동 커밋 정책이 있으면 Wave 단위로 커밋한다.
...
- Orchestrator가 `vulcan.py wave-complete <BW-ID>` 또는 `vulcan.py sync-session`으로 session 진행률을 갱신했거나, 작업자 runner가 갱신 필요 항목으로 보고했다.
