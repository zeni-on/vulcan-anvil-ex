...
Gate 3 테스트 설계나 Gate 4 QA처럼 상류 산출물을 많이 참고해야 하는 단계에서도, 상류 문서는 기본적으로 `reference_on_demand`에 둔다.

작업자 Run의 입력 계약은 넓은 문서 목록보다 좁은 작업 범위를 우선한다.
`traceability-review`, `independent-review`, `change-impact-analysis`처럼 보완/검수 성격의 Run은 Gate 전체 산출물을 `working_documents`에 모두 나열하지 않는다.
이 경우 `working_documents`에는 이번 Run이 직접 고치거나 판정할 핵심 문서만 두고, 나머지 Gate 산출물은 필요 시 디렉터리 또는 기준 문서로 찾아보는 대상으로 둔다.

`build-wave` 같은 작업자 Run은 Orchestrator 통합 Run과 분리한다.
작업자 Run의 `working_documents`에는 현재 Run 문서, 개발표준, 테스트케이스, 작업자가 직접 갱신할 증적/결과 문서만 둔다.
요구사항정의서, 전체 설계 산출물, 추적표는 기본적으로 `reference_on_demand`에 두고, related ID나 기준 충돌을 확인할 때만 읽는다.
작업자 Run의 `scope.writable`에는 담당 코드 경로, 담당 테스트 경로, 담당 증적 경로, 자기 Run 문서만 둔다.
`session.json`, Gate 상태, Wave 완료 상태, 릴리즈 승인서, 전체 추적표 갱신은 Orchestrator가 통합 단계에서 처리한다.

작업자 Run의 `verification.commands`에는 담당 영역 테스트, 빌드, 린트, `python vulcan.py run-check <현재 Run>`처럼 작업자 범위에서 재현 가능한 명령만 넣는다.
`python vulcan.py check-trace`, `python vulcan.py sync-session`, `python vulcan.py wave-start`, `python vulcan.py wave-complete`, `python vulcan.py session`은 Orchestrator 전용 명령이다.
작업자가 이 명령이 필요하다고 판단하면 직접 실행하지 않고 완료 보고의 Orchestrator 결정 필요 항목으로 반환한다.

worker worktree 결과는 손으로 복사하지 않는다.
Orchestrator는 `python vulcan.py run-integrate --run-id RUN-NNN --dry-run`으로 변경 파일과 scope 위반을 먼저 확인하고, 위반이 없을 때만 `--apply`로 허용 diff를 반영한다.
`run-integrate`가 실패하면 Orchestrator가 직접 고치지 않고 재작업 Run, QA Fix Run, Traceability Run, FIND/CR/ISSUE 중 하나로 분기한다.
...
`worker_execution_policy`는 Run이 subagent, Codex CLI, Claude CLI, manual worker처럼 Orchestrator와 분리된 작업자 runner에게 전달될 때 적용한다.
작업자 runner는 Run 계약 수행자가 될 수 있지만 Orchestrator가 아니다.
따라서 Gate 전환, `session.json`의 `current_gate`/`gate_status`/`completed` 변경, 사용자 승인/QA Pass/릴리즈 승인/merge 가능 여부의 최종 판단을 하지 않는다.
Gate 종료 보고 시에는 다음 Gate 진행 질문을 남기고 승인 대기 상태로 멈춘다. 사용자 명시 승인 전에는 `session --status done`을 실행하지 않고, 필요한 경우 Orchestrator가 `session --status awaiting-approval`만 기록한다.
이 판단이 필요하면 완료 보고의 `orchestrator_decision_needed`, `open_issues`, `findings`에 남긴다.
...
10. 작업자 runner로 실행 중이면 Gate 전환, session 변경, 최종 승인 판단을 하지 않고 Orchestrator 결정 필요 항목으로 반환한다.
11. RUN_OUTPUT_CONTRACT 형식으로 결과를 보고한다.
12. 현재 Gate 산출물 완료 후 다음 Gate 진행 승인을 질문하고 대기한다.
...
- 작업자 runner가 Gate 전환, session 변경, 최종 승인 판단을 하지 않는다.
