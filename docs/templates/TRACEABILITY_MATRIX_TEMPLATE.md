...
- 복수 ID와 증적 경로는 쉼표(`,`)로 구분하는 것을 원칙으로 한다. 사람이 읽기 쉽게 `/`를 쓰는 경우에는 `PGM-001 / API-001`처럼 양쪽에 공백을 둔다. 경로 내부의 `/`는 그대로 둔다.
- `증적` 칸에 여러 파일 경로를 넣을 때는 `backend/app/main.py, backend/tests/test_main.py, docs/artifacts/04-review/evidence/qa-001/QA-CMD.log`처럼 쉼표로 구분한다.
- Gate 3 테스트케이스 문서의 `Planned`는 계획 상태다. Gate 4의 실제 실행 결과는 `DOC-QA-G4-002_Test-Result_v0.1.md`에 기록하고, QA-003에서 이 결과를 근거로 본 추적표의 `상태`, `증적`, `요구사항별 검증 요약`을 갱신한다.
...
| `Implemented` | 구현 완료 |
| `Verified` | Gate 4 테스트 결과서와 증적 기준 검증 완료 |
| `Changed` | 변경 발생 |
| `Deferred` | 후속 범위로 이관 |
| `Rejected` | 제외 또는 반려 |
...
- `Verified`: QA 테스트 결과서에서 관련 필수 `UT/IT/UI/PT`가 Pass이고, 필요한 로그/화면/계약 증적이 연결되어 있다.
- `Fail`: QA 테스트 결과서에 Fail이 있거나 기대 결과와 증적이 불일치한다.
- `Not Run` 또는 `environment_blocked`: 테스트 결과서에 미실행 또는 환경 차단 사유가 남아 있다.
- `Deferred`: 승인된 후속 범위로 이관했다.
- `Changed`: 요구사항, 설계, API, DB, 보안, 화면 계약 변경이 필요해 CR 후보 또는 승인 변경으로 분류했다.
