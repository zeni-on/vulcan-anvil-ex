...
프로그램명세서는 API와 처리 프로그램의 책임을 요약하고, 본 API 정의서는 클라이언트와 서버가 함께 따라야 하는 요청/응답 계약을 상세화한다.
...
- API가 없는 프로젝트는 본 문서를 작성하지 않거나 `상태: N/A`로 두고, 프로그램명세서에 `API 정의서: 해당없음`과 사유를 남긴다.
