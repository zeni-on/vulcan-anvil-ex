...
- 프로그램 내부 구조가 단순 CRUD를 넘어가면 상세 SW 설계 다이어그램을 작성한다.
- 상태 전이가 있으면 상태 다이어그램, 복잡한 도메인 객체/정책이 있으면 클래스 다이어그램, 외부 연계/비동기 흐름이 있으면 시퀀스 또는 액티비티 다이어그램을 작성한다.
- 다이어그램이 불필요하면 `해당없음`으로 두지 말고 생략 사유를 명시한다.
...
상세 SW 설계 다이어그램은 모든 프로그램에 기계적으로 작성하지 않는다. 아래 판단표에서 필요성이 있으면 작성하고, 불필요하면 생략 사유를 남긴다.

| PGM-ID | 복잡도 | 상태 전이 | 도메인/정책 복잡도 | 외부/비동기 연계 | 권장 다이어그램 | 필요 여부 | 생략 사유 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 조건 | 권장 다이어그램 |
| --- | --- |
| 도메인 객체, 정책, 전략, 권한 판단이 복잡함 | Class Diagram |
| 상태값과 전이 조건이 업무 핵심임 | State Diagram |
| 여러 프로그램/API/외부 시스템이 순서대로 협력함 | Sequence Diagram |
| 배치, 워크플로우, 조건 분기, 재처리 흐름이 중요함 | Activity Diagram |
...
| 복잡한 프로그램에 상세 SW 설계 다이어그램 또는 생략 사유가 작성되었는가 |  |
