...
<!-- Source: https://github.com/<owner>/<repo> (MIT License, © YYYY <Author>). Keep this attribution with the imported file. -->
...
가져온 파일 안에 출처, upstream 라이선스, 저작권자를 주석 또는 본문으로 남긴다.
필요하면 Apache-2.0의 `NOTICE` 파일을 추가해 배포 시 함께 전달한다.
...
- [ ] (외부 출처) 파일 안에 출처, upstream 라이선스, 저작권자 고지 보존
...
- Vulcan 내부 참고:
  ...
  - 라이선스 관리: 파일 내부 출처 고지 또는 필요 시 Apache-2.0 `NOTICE`
