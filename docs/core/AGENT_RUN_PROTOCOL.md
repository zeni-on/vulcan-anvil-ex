...
| 개발표준 검수 | `development-review` | 언어/프레임워크, 패키지 구조, 메시지 관리, 주석/코딩/테스트 컨벤션, 빌드/실행 명령 |
...
| Gate 2 Review | `screen-review` | 화면 누락, 상태 누락, 시안 연결, 증적 가능 여부 검토 |
...
| 조건 | 최소 확인 |
| --- | --- |
| 개발표준 준수 | 개발표준정의서가 Draft가 아니고 언어, 구조, 메시지, 주석, 테스트, 보안 기준을 가진다. |
| 구현 파일 연결 | 요구사항추적표의 증적에 실제 구현 파일 경로가 연결된다. |
| 추적 ID 표시 | 구현 또는 테스트 파일 안에 관련 `REQ`, `PGM`, `SCR`, `SEC`, `UT`, `IT` 중 하나 이상의 ID가 남는다. |
| 테스트 실행 | Gate 3에서 정의한 테스트가 실행되고 Pass/Fail/Skip/미실행 상태가 기록된다. |
