...
| G2-03 | Screen / User Flow | 사용자가 보는 화면과 상태 흐름을 확정한다 | REQ/AC, 프로토타입, 기존 화면 | SCR, UIREF, 화면 상태, 메시지 위치, 사용자 흐름 |
...
- 프로토타입, Figma, 이미지 시안, 기존 화면 캡처를 `참고`라고만 두고 구현 기준을 모호하게 넘기지 않는다.
...
## 7. Prototype To Implementation Contract

프로토타입 또는 외부 시안은 Gate 2에서 다음 중 하나로 분류한다.
