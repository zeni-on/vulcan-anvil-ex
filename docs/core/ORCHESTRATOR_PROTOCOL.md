...
Gate 5 승인 전까지 `main`에 구현 결과를 직접 누적하지 않는다. Gate 5에서 릴리즈 승인 문서, QA 결과, 잔여 Backlog를 확인한 뒤 `workflow.integration_branch`를 `main` 또는 `workflow.release_merge_to`에 반영한다.
