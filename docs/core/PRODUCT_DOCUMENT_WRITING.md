...
### 1.2 Brief의 목차와 반복 갱신

`PRODUCT_BRIEF.md`는 제품의 목적·사용자·경계·핵심 능력을 설명하는 **Vision 역할의 원본 한 개**다. 별도 Vision/파일럿 Brief를 필수로 만들지 않는다. 문서 전체를 현재 기준으로 관리하되 제품 정의와 이번 작업 선택을 구분한다. 합의한 제품 방향에 아직 구현하지 않은 기능이 포함될 수 있으며, 정의·범위 합의·구현 허가·인수 결과는 서로 다르다.

| 고정 목차 | 작성할 내용 | 상세 원본으로 보낼 내용 |
| --- | --- | --- |
| 1. 문제·목표·성공 기준 | 해결할 문제, 제품 가치, 관측할 효과, 비목표 | 개별 AC와 시험 결과 |
| 2. 사용자·업무 맥락·제품 경계 | 역할·사용 환경, 제품/사람/외부 시스템의 책임 | 개별 사업 자료 목록, 상세 권한·운영 규칙 |
| 3. 핵심 시나리오·전체 흐름 | 시작 계기·참여자·인계·종료 결과와 시나리오 간 관계, 상세 진입 링크 | 단계별 예외, REQ/AC·API/DB/UI 상세 |
| 4. 이번 작업 범위 | 출발 기준, 이번 목표·추가/변경/제거, 포함·제외·후속, 검증 원본 링크 | 상세 작업 지시, 테스트 정의·로그, 총괄의 CLI 실행 메모 |
| 5. 핵심 제약·가정·열린 결정 | 중요한 미정 사항의 영향과 확인 담당/시점 | 모든 대화·질문의 누적 원장 |
| 6. 관련 원본·과거 기록 | 공통 근거와 당시 범위·결과의 기존 기록 링크 | 과거 본문 재복사, 버전별 변경 전문 |

새 양식은 제목과 목차 링크를 고정하고 작업명은 4절 본문에 적는다. 이는 작성 위치 안내이지 새 필수 필드/승인 단계/검사 차단 조건이 아니다. 기존 Brief의 목차를 자동 재배치하거나 검사 통과를 위해 일괄 변경하지 않는다.

기능 목록만으로 업무 흐름을 대신하지 않는다. 누가 무엇을 계기로 시작하고 누구에게 인계해 어떤 결과로 끝내는지 이해할 수 있는 상위 설명은 Brief에 남긴다. 분량을 줄이려고 이 맥락까지 링크 뒤로 숨기거나, 이번 범위를 정하기 전에 모든 미래 흐름을 상세 설계하지 않는다.

기능 확장 때는 영향받는 원본만 고친다. 사용자·목표·책임·핵심 흐름이 바뀌면 1~3절도 수정하며, 단순히 다음 버전을 시작했다는 이유로 전체를 다시 쓰지 않는다. 미합의 변경은 기존 candidate 표식으로 분리하고 합의 후 해당 절에 반영한다. 정의를 넓혀도 이번 구현 범위가 자동 확대되지는 않는다.

파일럿/이번 범위가 끝나면 당시 범위·검증 결과·남은 한계를 기존 결과 기록에 보존한다. 실제 발행한 경우에만 릴리즈 기록을 연결한다. 다음 범위가 합의되면 4절을 교체하고 이전 결과는 출발 기준 또는 6절의 history 링크로 남긴다. 다음 범위가 없으면 종료 결과와 미정 상태를 유지한다. 파일럿마다 릴리즈·새 Brief·새 이력 문서를 의무 생성하지 않는다. 이전 결과는 새 변경의 검증을 대신하지 않으며 미완료 의무도 사라지지 않는다.

`session.json`이 실행 상태·현재 작업 범위의 원본이며 Brief는 범위 합의의 근거다. 현재 scope가 Brief 내용을 참조한다면 문서 변경 뒤 해당 참조와 영향을 총괄이 재검토한다. 헤딩/앵커 이동과 기존 승인 내용의 재배치는 5절을 따르며, `upgrade`는 새 양식/지침만 설치하고 작성된 Brief는 보존한다. 표준·실무 가이드의 참고 근거는 [Discovery Guide](https://github.com/zeni-on/vulcan-anvil-ex/blob/main/docs/reference/PRODUCT-DISCOVERY-AND-VALIDATION-GUIDE.md#2-참고-기반)에 둔다.
...
| --- | --- | --- | --- |
| 목표·이번 범위 / 총괄·제품 | `PRODUCT_BRIEF.md` | 제품 정의와 이번 범위는 원장에 유지 | `PRODUCT_BRIEF_TEMPLATE.md`: 1.2절의 고정 목차와 반복 갱신 기준 |
