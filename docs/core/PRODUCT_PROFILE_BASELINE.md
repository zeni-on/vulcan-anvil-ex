...
전환 검사는 실제 실행 Wave와 계획 본문의 언급을 구분한다. 기존 승인 기록과 Git snapshot으로 확인되는 완료 Run은 과거 이력으로 취급하며, 새로 만들거나 변경한 Run까지 승인된 것으로 보지 않는다. 현재 검증 결과는 과거 실패/재시도 이력과 구분하고, 애매한 옛 서술을 맞추려고 일괄 재작성하지 않는다. 현재 필수 검증의 실패·누락이나 등록된 미완료 작업은 계속 차단한다.
