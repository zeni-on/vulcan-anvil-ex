...
11. UI 검증이 있으면 상태/시나리오별 UI-ID와 캡처 증적을 1:1로 연결한다.
12. 범위가 너무 커서 완결 구현이 어렵다면 중간 구현을 완료 처리하지 않고 Orchestrator 결정 필요 항목으로 반환한다.
13. Gate 전환, session 변경, 최종 승인 판단을 하지 않고 Orchestrator 결정 필요 항목으로 반환한다.
14. RUN_OUTPUT_CONTRACT 형식으로 결과를 보고한다.
