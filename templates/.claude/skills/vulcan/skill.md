...
1. 에이전트 작업 완료 보고를 받으면 (또는 모든 청크 완료 시), **산출물 파일이 실제로 존재하고 내용이 템플릿이 아닌지 검증**한다
...
2. 산출물 검증 통과 후, **사용자에게 상세 보고**한다:
   - 완료된 Gate/단계 이름
   - 생성·수정된 산출물 파일 목록
   - 주요 내용 요약 (예: REQ 수, 설계 구조, 테스트 수, QA 판정 결과 등)
   - **"다음 Gate로 진행할까요?"** 라고 명시적으로 승인을 요청한다

3. **⛔ 반드시 사용자의 응답을 기다린다.**
   - 사용자가 "응", "진행", "다음" 등 명시적 승인을 할 때까지 **절대 다음 작업을 시작하지 않는다**
   - 한줄 보고 후 바로 다음 Gate로 넘어가는 것은 **금지**
   - 사용자가 수정 요청 시 → 해당 에이전트 재투입 → 수정 완료 후 다시 보고
