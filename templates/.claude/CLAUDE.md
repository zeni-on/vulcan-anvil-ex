...
- `python vulcan.py check-trace` — Gate 추적성 검사 (Gate 완료 시 자동으로 SW 아키텍처 baseline 검증 포함)
- `python vulcan.py check-architecture --level {draft|baseline}` — SW 아키텍처 성숙도 검증. Gate 2 초반에는 `draft`, Gate 3 진입 전에는 `baseline`
- `python vulcan.py session --gate <gateN> --status done --feature "..."` — Gate 상태 갱신
- `python vulcan.py gate-start <gateN>` — 승인된 CR을 처리하기 위해 해당 Gate를 다시 진행 상태로 전환 (이전 `rollback` 명령은 제거됨. CR 상세서와 Run 문서의 scope로 영향 범위 관리)
...
- "QA 리뷰" / "Gate 4" → `review`
- "추적성 확인", "다음 Gate로", "CR 처리/Gate 재진행" → orchestrator 직접 (`gate-start` + Run 문서)
