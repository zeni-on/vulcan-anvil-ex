 ...
 * - UT-012-18: template='A2'일 때 LayoutA2가 렌더링되고 LayoutA/LayoutB는 렌더링되지 않음
 * - UT-012-18B: template='B'일 때 LayoutB가 렌더링되고 LayoutA는 렌더링되지 않음
 *
 * 구현 전략:
...
function LayoutSwitchUnderTest({ initialTemplate }: { initialTemplate: LayoutTemplate }) {
  ...
  function toggle() {
    setTemplate(prev => prev === 'A' ? 'A2' : prev === 'A2' ? 'B' : 'A')
  }
...
describe('UT-012-18: template=A2일 때 LayoutA2 렌더링, LayoutA/LayoutB 미렌더링', () => {
  it('template=A2이면 data-testid="layout-a2"가 렌더링된다', () => {
    render(<LayoutSwitchUnderTest initialTemplate="A2" />)
    expect(screen.getByTestId('layout-a2')).toBeInTheDocument()
  })

  it('template=A2이면 LayoutA와 LayoutB가 렌더링되지 않는다', () => {
    render(<LayoutSwitchUnderTest initialTemplate="A2" />)
...
describe('UT-012-18B: template=B일 때 LayoutB 렌더링, LayoutA 미렌더링', () => {
...
describe('UT-012-19: LayoutToggle 클릭 후 레이아웃 즉시 전환 (페이지 리로드 없음)', () => {
  it('A에서 LayoutToggle 클릭 후 layout-a2가 렌더링된다 (페이지 리로드 없음)', () => {
    render(<LayoutSwitchUnderTest initialTemplate="A" />)
    ...
    // 전환 후: LayoutA2 렌더링, LayoutA 미렌더링
    expect(screen.getByTestId('layout-a2')).toBeInTheDocument()
    expect(screen.queryByTestId('layout-a')).not.toBeInTheDocument()
  })

  it('A2에서 LayoutToggle 클릭 후 layout-b가 렌더링된다 (페이지 리로드 없음)', () => {
    render(<LayoutSwitchUnderTest initialTemplate="A2" />)
    ...
    expect(screen.getByTestId('layout-b')).toBeInTheDocument()
    expect(screen.queryByTestId('layout-a2')).not.toBeInTheDocument()
  })
