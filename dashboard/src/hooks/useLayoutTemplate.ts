 ...
 *
 * localStorage 키 "vulcan-dashboard-layout"에 'A' | 'B' 값을 영속화한다.
 * SSR 환경(typeof window === 'undefined')에서는 기본값 'A'를 반환한다.
 *
 * 보안 (SEC-012-01): localStorage 읽기 시 'A' 또는 'B'만 허용하고
...
/** 지원하는 레이아웃 템플릿 값 */
export type LayoutTemplate = 'A' | 'B'
 ...
 * 'A' 또는 'B' 외의 값은 기본값 'A'로 폴백한다 (SEC-012-01).
 */
 ...
 * - toggle(): 'A' ↔ 'B' 전환 + localStorage 즉시 저장
 *
...
export function useLayoutTemplate(): { template: LayoutTemplate; toggle: () => void } {
  ...
  /**
   * 템플릿을 A ↔ B로 전환하고 localStorage에 저장한다.
   */
  function toggle() {
    setTemplate((prev) => {
      const next: LayoutTemplate = prev === 'A' ? 'B' : 'A'
